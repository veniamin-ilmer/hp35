...
impl Keyboard {
  ...
  pub(super) fn run_refresh_cycle(&self, board: &mut Board) {
    ...
    if let Some(click_float) = click_var.as_f64() {
      ...
      if code == -1.0 { //Depress event
        board.cnt.status[0] = false;
        board.cnt.current_keypress = None;
      } else if code == 255.0 { //Hack around javascript not able to handle 0.
        board.cnt.status[0] = true;  //The only status flag connected to hardware. (Set with key press).
        board.cnt.current_keypress = Some(u6::new(0));
      } else {
        board.cnt.status[0] = true;  //The only status flag connected to hardware. (Set with key press).
        board.cnt.current_keypress = Some(u6::new(code as u8));
      }
    }
  }
}
